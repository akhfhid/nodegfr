...
const makeRequest = (description, params) => {
    return new Promise((resolve) => {
        ...
        // if (params.includeDraft) formData += `&draftResponse=%5Bnull%2Cnull%2C%22${FBZX}%22%5D`; // OMITTING DRAFT
        if (params.pageHistory) formData += `&pageHistory=${params.pageHistory}`;

        // User Provided Values (SAME AS BEFORE)
        formData += '&entry.1152921546=tset0_NO_DRAFT';
        formData += '&entry.1515637797=Opsi+1';

        formData += '&entry.577699138=Opsi+1';
        formData += '&entry.476679252=Opsi+1';

        formData += '&entry.948531731=test67_NO_DRAFT';
        ...
        const options = {
            ...
            headers: {
                ...
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) test v3 no draft'
            }
        };
...
(async () => {
    // TEST: Continue=1, PageHistory=ALL, NO draftResponse
    await makeRequest("With Continue, NO Draft", {
